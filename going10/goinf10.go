...
// ProcessMap: inode → process string with caching
type ProcessMap map[string]string

var (
	procMapCache      ProcessMap
	procMapLastUpdate time.Time
	procMapMutex      sync.Mutex
)

// getProcessMap with caching to reduce I/O in watch mode
func getProcessMap(refreshInterval time.Duration) ProcessMap {
	procMapMutex.Lock()
	defer procMapMutex.Unlock()

	now := time.Now()
	if procMapCache == nil || now.Sub(procMapLastUpdate) > refreshInterval {
		procMapCache = buildProcessMap()
		procMapLastUpdate = now
	}
	return procMapCache
}
...
func buildProcessMap() ProcessMap {
	...
	for _, fdPath := range matches {
		...
		// Try comm first, then fall back to cmdline
...
// parseHexIPPort decodes /proc hex IP:port (little endian) with enhanced IPv6 support
func parseHexIPPort(s string, isIPv6 bool) (string, int, error) {
	...
	// Handle IPv6 addresses which have more colons
...
func validateFilters(state, localIP, remoteIP string) error {
	if state != "" {
		...
		if !valid {
			return fmt.Errorf("invalid state filter: %s. Valid states: %v", state, getValidStates())
		}
	}
...
// resolveHosts: optional reverse DNS lookups with concurrency limit
func resolveHosts(sockets []Socket, timeout time.Duration, maxConcurrent int) {
	...
	for i := range sockets {
		...
		go func(idx int) {
			...
			}(sockets[idx].RemoteIP)

			select {
			case r := <-done:
				if r != "" {
					sockets[idx].Resolved = r
				}
